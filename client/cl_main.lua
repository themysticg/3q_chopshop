...
Citizen.CreateThread(function()
    while true do
        ...
        if scrapblip then
            ...
            if not mert then
                print(scanner)
                exports['3q_chopshop']:SetupDigiScanner(scanner, {})
                mert = true
            end
            if dist <= 3 then
                if not copsCalled then
                    ...
                    TriggerServerEvent("3q_chopshop:server:callCops", "Chopshop", 0, streetLabel, pos)
                    copsCalled = true
                end
                Citizen.Wait(5000)
                QBCore.Functions.Notify(Config.Locale["FoundVeh"], 'success')
                exports["3q_ui"]:Show(Config.Locale["title"], Config.Locale["chop3"])
                Citizen.Wait(10000)
...
RegisterNetEvent('3q_chopshop:jobaccept')
AddEventHandler('3q_chopshop:jobaccept', function()
...
function SpawnVehicle(vehicle, x, y, z, w)
    ...
    QBCore.Functions.SpawnVehicle(vehicle, function(veh)
        ...
        exports["3q_ui"]:Show(Config.Locale["title"], Config.Locale["chop1"])
        ...
        if Config.Email == true then
            ...
            exports["3q_ui"]:Show(Config.Locale["title"],
...
function ScrapVehicle()
    ...
    if GetVehicleNumberPlateText(veh) ~= LicensePlate then
        ...
        exports["3q_ui"]:Show(Config.Locale["title"], Config.Locale["chop4"])
        StartChopping()
        DeleteBlip()
    end
end
...
RegisterNetEvent('3q_chopshop:client:robberyCall')
AddEventHandler('3q_chopshop:client:robberyCall', function(type, key, streetLabel, coords)
...
function StartAnimation(k)
    if Config.CarTable[k].anim == "wheel1" or Config.CarTable[k].anim == "wheel2" or Config.CarTable[k].anim == "wheel3" or Config.CarTable[k].anim == "wheel4" then
        TriggerEvent('3q_chopshop:wheelanimation')
        Citizen.Wait(7000)
    ...
    elseif Config.CarTable[k].anim == "trunk" then
        ...
        TriggerEvent('3q_chopshop:trunkanimation')
        Citizen.Wait(2000)
        Config.CarTable[k].chopped = true
        TriggerServerEvent('3q_chopshop:server:rewardplayer', Config.CarTable[k].anim)
        Citizen.Wait(1000)
    ...
    elseif Config.CarTable[k].anim == "hood" then
        SetVehicleDoorOpen(vehicle, Config.CarTable[k].destroy, false, true)
        TriggerEvent('3q_chopshop:hoodanimation')
        Citizen.Wait(4000)
        SetVehicleDoorBroken(vehicle, Config.CarTable[k].destroy, true)
        Config.CarTable[k].chopped = true
        TriggerServerEvent('3q_chopshop:server:rewardplayer', Config.CarTable[k].anim)
...
function UpdateBars(dist, plate)
    ...
    if dist < 1.0 then
        ...
        -- end
        if not mert then
            exports['skillchecks']:untangle(function (result)
                if result then
                    TriggerEvents()
